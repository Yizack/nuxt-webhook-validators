...
export const isValidGithubWebhook = async (event: H3Event): Promise<boolean> => {
  ...
  const key = await subtle.importKey('raw', encoder.encode(secretKey), algorithm, extractable, ['verify'])
  const isValid = await subtle.verify(algorithm.name, key, Buffer.from(webhookSignature, 'hex'), encoder.encode(body))
